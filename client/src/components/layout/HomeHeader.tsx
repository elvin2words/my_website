...
const Header: React.FC = () => {
  ...
  const NavDropdown = ({
    ...
    >
      <button className={`flex items-center font-medium text-lg text-white hover:${accentClass}`}>
        <span className={active ? accentClass : ""}>{label}</span>
        <ChevronDown
          className={`ml-1 w-4 h-4 transition-transform ${openMenu === id ? "rotate-180" : ""}`}
        />
      </button>
      ...
      <AnimatePresence>
        {openMenu === id && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 bg-primary border border-white/20 rounded-lg shadow-lg p-2 z-50"
          >
            {links.map((l) => (
              <Link
                href={l.href}
                key={l.href}
                className={`block px-4 py-2 text-white hover:${accentClass} hover:bg-white/10 rounded`}
                onClick={() => {
                  setOpenMenu(null);
      ...
      <span className={`absolute -bottom-1 left-0 w-0 h-0.5 ${accentClass} transition-all duration-300 group-hover:w-full`} />
    </div>
  );
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex justify-between items-center w-full lg:hidden">
          ...
          <div className="flex items-center">
            <Link href="/" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="flex items-center text-xl font-poppins font-bold tracking-wide">
              ...
              {/* <span className="ml-1">{typingRef.current ? "|" : ""}</span> */}
            </Link>
          </div>
        ...
        <div className="hidden lg:grid grid-cols-3 items-center w-full mx-auto max-w-7xl relative">
          {/* Column 1: Logo (Left aligned) */}
          <div className="flex items-center">
            <Link href="/" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="flex items-center text-xl font-poppins font-bold tracking-wide">
              <Cat className="w-6 h-6 mr-2" />
              <span>{displayText}</span>
              <span className={`ml-1 ${isTyping ? 'opacity-100' : 'opacity-0'} transition-opacity duration-100`}>_</span>
            </Link>
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 border-b border-white border-opacity-20"></div>
      </header>
